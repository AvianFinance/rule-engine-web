...
import rent_token from "../config.js"
...
const UpfrontRental = () => {
    ...
    const Check = async () => {
        ...
        await checkFunction('rent', JSON.stringify({ eventsList: eventsList, errorsList: errorsList, modifiersList: modifiersList, functionlist: bodyvalues }))
            .then((res) => {
                console.log(res)
                if(res.data){
    ...
    const Deploy = async () => {
        setisloading(true)
        if(isConnected){
            await deployContract('rent', JSON.stringify({ eventsList: eventsList, errorsList: errorsList, modifiersList: modifiersList, functionlist: bodyvalues }))
				.then(async (res) => {
                    console.log(res)
                    if (res.data) {
                        ...
                        try {
                            const _marketplace = new ethers.Contract( // We will use this to interact with the AuctionManager
                                "0x9bF19331CF09Eb6A693D78AeEb39A4E0C21113e1",
                                RentProxy.abi,
                                signer
                            );
                    ...
                    <Grid item xs={12} md={12} >
                        <div>
                            ...
                            <Accordion expanded={expanded === 'panel4'} onChange={handleChange('panel4')}>
                                ...
                                <AccordionDetails>
                                        <Functiondetails contract_type={'rent'} bodyvalues={bodyvalues} setbodyvalues={setbodyvalues}/>
                                </AccordionDetails>
                            </Accordion>
                        </div>
                    </Grid>
                    ...
                    <>
                        <Grid item xs={12} md={6}>
                            ...
                            <iframe 
                                ...
                                src={lattestContratval ? lattestContratval.address : "https://jade-witty-wolverine-528.mypinata.cloud/ipfs/QmVmMQ3L6kSyrA4XZuuSWg1fZYQtnZEqqujA6Bzw1dtsrY?pinataGatewayToken=xA9loW9mwlHFcUpNy4slXCUcWXNb-o8w5UN3T8pDiJRXLEh2zw4NoFeijQv9m5kK&_gl=1*u1cr9n*rs_ga*MTAxNjE0NTAyLjE2ODYwNjg1NTc.*rs_ga_5RMPXG14TE*MTY4Njg0OTM4MC4xMy4xLjE2ODY4NDk4ODAuMTMuMC4w"}>
                            </iframe> 
                            {/* <p>hii</p> */}
                        </Grid>
                        <Grid item xs={12} md={6}>
